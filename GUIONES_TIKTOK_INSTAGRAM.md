# Guiones TikTok / Instagram — primer lote

*10 vídeos listos para grabar ya, sin esperar a que pase nada más. Todos parten de hechos reales de hoy — nada de dossiers enviados, respuestas de galerías o ventas, porque todavía no han ocurrido. Cuando ocurran de verdad, se añaden aquí como nuevos guiones (ver `SERIE_DOCUMENTAL.md`).*

**Formato técnico general**: vertical 9:16, 15-45 segundos la mayoría, subtítulos siempre (gran parte se ve sin sonido), hook en los primeros 1-2 segundos —sin intro de marca, sin cuenta atrás, directo a la frase.

---

### 1. Tengo dos pinturas y una pregunta
**Pilar**: Descubrimiento · **Hook**: Experimento

- **Gancho** (dilo mirando a cámara, sin editar nada encima): "Tengo dos pinturas. No tengo galería. No tengo coleccionistas. Solo tengo una pregunta."
- **Beats**: muestra las dos obras (planos cortos, Umbral y Habitar) → "¿Se puede construir una carrera artística desde cero?" → "Voy a documentarlo mientras ocurre, no cuando ya haya terminado."
- **Texto en pantalla**: la pregunta, en Cormorant Garamond si tu editor lo permite.
- **Caption**: "Empiezo a documentar esto en público. Dentro de un tiempo vuelvo a este vídeo y vemos qué pasó de verdad. ¿Qué es lo primero que te gustaría ver del proceso?"

---

### 2. Por qué volví a pintar
**Pilar**: Descubrimiento · **Hook**: Proceso

- **Gancho**: "Durante años encontraba una razón para no volver al lienzo."
- **Beats**: qué cambió (deja de posponerlo, compra los lienzos sin plan) → conecta con Manifiesto §2 ("necesitaba comprender un mundo que sentía complejo, lleno de detalles que otras personas parecen pasar por alto") → cierre en calma, sin resolverlo del todo.
- **Caption**: "No fue un plan. Fue dejar de esperar el momento perfecto."

---

### 3. La pregunta que respondí antes de pintar nada
**Pilar**: Descubrimiento · **Hook**: Proceso

- **Gancho**: "Antes de tocar un pincel tuve que responder algo."
- **Beats**: "¿Qué quiero que sienta alguien al ver mi obra?" → no buscaba un paisaje, ni una técnica, ni un estilo → buscaba una sensación → esa respuesta es la que sostiene todo lo que haces ahora (curiosidad, calma, contemplación — Manifiesto §8).
- **Caption**: "No fue una técnica lo que definió mi obra. Fue esta pregunta."

---

### 4. Cómo sé que una pintura está terminada
**Pilar**: Descubrimiento · **Hook**: Proceso

- **Gancho**: "Durante años terminaba una obra y encontraba algo que cambiar."
- **Beats**: qué fue distinto con Umbral o Habitar (la observas y no quieres tocar nada, no porque sea perfecta, sino porque se siente viva) → "Dejé de buscar un estilo. Empecé a buscar una experiencia."
- **Nota**: usa la obra real y la historia real que ya está escrita en `obras/umbral/index.html` o `obras/habitar/index.html` — no inventes una anécdota nueva, cuenta la que ya está documentada.
- **Caption**: la frase textual de la ficha de la obra + enlace.

---

### 5. Qué significa Hábitats
**Pilar**: Descubrimiento · **Hook**: Investigación/Proceso

- **Gancho**: "Esta colección no habla de lugares. Habla de cómo los habitamos."
- **Beats**: lee o parafrasea el statement real de la colección (`colecciones/habitats/index.html`) → muestra Umbral y Habitar lado a lado, mismo lenguaje visual (degradado, composición de horizonte), paleta distinta.
- **Caption**: statement completo o un fragmento, tal cual está escrito en la web.

---

### 6. Por qué publiqué una web con solo dos obras
**Pilar**: Descubrimiento · **Hook**: Experimento

- **Gancho**: "Solo tengo dos pinturas. Publiqué la web de todas formas."
- **Beats**: "Esperar a tener veinte obras no iba a responder ninguna pregunta. Publicarla sí." → enseña la web brevemente (scroll por la ficha de una obra).
- **Caption**: "mariaperezart.github.io/estudio — todavía en construcción, a propósito."

---

### 7. Lo difícil de fotografiar una pintura
**Pilar**: Investigación · **Hook**: Investigación

- **Gancho**: "Pensé que fotografiar un cuadro sería fácil. No lo fue."
- **Beats**: colores que cambiaban, texturas que desaparecían, la obra dejaba de parecerse a sí misma → "Una fotografía también forma parte de la obra." → opcional: muestra un antes/después de una foto editada (siempre que sea fiel al color real, nunca un filtro que lo distorsione — ver `MANUAL_DE_MARCA.md` §2).
- **Caption**: "No era la cámara. Era todo lo que no sabía sobre luz, ángulo y edición."

---

### 8. El certificado de autenticidad
**Pilar**: Conversión (con marco de proceso, no de venta) · **Hook**: Proceso

- **Gancho**: "Cada obra que vendo incluye esto."
- **Beats**: muestra el documento del certificado (`certificados/umbral/` o `certificados/habitar/`) → explica qué garantiza (pieza original, única, no reproducida) → por qué lo preparaste desde ahora, con solo dos obras, y no cuando ya tuvieras más.
- **Caption**: "Tratar esto como una carrera profesional empieza en los detalles que nadie ve todavía."

---

### 9. Cómo pienso el precio (con honestidad, no con misterio)
**Pilar**: Conversión · **Hook**: Investigación

- **Gancho**: "¿Cuánto cuesta una pintura cuando todavía no tienes trayectoria?"
- **Beats**: explica el método real usado (precio por pulgada cuadrada para artistas emergentes) → dilo tal cual: "2.400€, orientativo, todavía sin validar con una venta real" → por qué prefieres decir esto en vez de fingir una cifra fija e inamovible.
- **Nota**: sigue la plantilla de precio de `MANUAL_DE_MARCA.md` §3 — nunca inventar escasez o demanda.
- **Caption**: "Prefiero ser transparente con el proceso que aparentar una certeza que no tengo todavía."

---

### 10. Mi estudio es este rincón de mi casa
**Pilar**: Conexión · **Hook**: Proceso

- **Gancho**: "Esto no es un estudio de galería. Es un rincón de mi casa."
- **Beats**: plano del espacio real donde trabajas → por qué eso no es un problema para tomarte esto en serio → conecta con la idea de "comportarse como profesional antes de que el mercado te trate como uno" (sistemas, documentación, certificados) sin necesitar un espacio grande todavía.
- **Caption**: "No vas a ver un estudio de revista. Vas a ver el proceso real."

---

## Cómo seguir después de estos 10

- Cuando exista una investigación real de mercado (`investigador-mercado`), se añade un guion de tipo Investigación con datos reales.
- Cuando envíes un dossier de verdad, se escribe el guion de Experimento **en ese momento**, no antes — con la plantilla de `BIBLIA_DEL_UNIVERSO.md` §12 (pregunta / hipótesis / qué hiciste / qué pasó).
- No fuerces un vídeo nuevo si esta semana no hay nada real — revisa `MANUAL_DE_MARCA.md` §8.
